...
{/* AUTHORED page (OCT-1512; agents funnel OCT-1534) — hand-written in the
    octolens/octolens repo under docs/cli/authored/scripting/ and published by
    `pnpm cli:docs:site`. Every `octolens` line below is docs-lint-verified
    against the live command registry; the surface-selection guidance here is
    aligned with the Agent Skill overview page (/skill/overview) — change one,
    check the other. */}
...
An agent does not need a pre-provisioned account: `octolens signup
--headless` creates a workspace end to end from a
[headless](/cli/concepts/output-modes#headless) shell — email, emailed
...
upgrade`) are all on [Signup & the Agents plan](/cli/agents).
