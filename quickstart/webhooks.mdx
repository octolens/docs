...
<Warning>
The mention fields in the payload (`title`, `body`, `author`, …) are third-party content from the open internet — treat them as untrusted input. Escape before rendering, parameterize before storing, and if you forward mentions to an LLM or AI agent, keep the text in a delimited data context: it can contain prompt injection. See the [API security notes](/api/v2/overview#security-treat-mention-content-as-untrusted-input).
</Warning>
...
| `data.review` | object | Only present for [customer review](/learn/platforms/customer-reviews) mentions (`source` is `trustpilot_reviews`, `app_store_reviews`, `google_play_reviews`, or `google_reviews`). Contains `rating` (number), `ratingMax` (number, usually 5), `verified` (boolean or null), `response` (owner or developer reply, string or null), and `responseAt` (ISO 8601 string or null). |
